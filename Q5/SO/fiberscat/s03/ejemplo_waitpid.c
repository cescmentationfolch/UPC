...
int main(int argc,char *argv[])
{
	...
	while (hijos > 0)
	{
		...
		write(1, buff, strlen(buff));
		if(res==0) sprintf(buff,"Este hijo ha fallecido por causas naturales con exit()",pid);
		else sprintf(buff,"Este hijo ha fallecido de forma anormal mediante un signal, exit status: %d\n",pid,res);
		write(1, buff, strlen(buff));
		hijos --;
		contador++;
	}
